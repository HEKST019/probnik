        ...
        try {
            ...
            if (empty($post_id)) {
                ...
                try {
                    $stmt = $pdo->prepare("INSERT INTO st_wc_product_meta_lookup
                        (product_id, sku, virtual, downloadable, min_price, max_price,
                         ...
                         total_sales, tax_status, tax_class, calories)
                        VALUES
                        (?, ?, 0, 0, ?, ?, ?, ?, ?, 0, 0, 0, 'taxable', '', ?)");
                    ...
                    $stmt->execute(array($post_id, $sku, $min_max_price, $min_max_price, $onsale, $stock_quantity, $stock_status, $calories));
                } catch (Exception $e) {
                    // Игнорируем ошибку если таблицы нет
                    error_log("WC meta lookup table not found: " . $e->getMessage());
                }
                ...
                if (create_product_page($post_id, $product_data, $pdo)) {
                    $message = "Товар успешно добавлен! ID: " . $post_id . " (Страница создана)";
                } else {
                    $message = "Товар успешно добавлен! ID: " . $post_id . " (Ошибка создания страницы)";
                }
                $message_class = "success";
