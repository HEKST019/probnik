...
// Вставляем заказ
$sql_stats = "INSERT INTO st_order_stats
...
if ($mysql->query($sql_stats)) {
    ...
    // Вставляем адрес
    $sql_address = "INSERT INTO st_order_addresses
    ...
    if ($mysql->query($sql_address)) {
        ...
        unset($_SESSION['cart']);
        echo "Заказ успешно оформлен! Номер заказа: $order_id. Очень скоро с вами свяжется наш менеджер.";
    } else {
