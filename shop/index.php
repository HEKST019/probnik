...
if (isset($_GET['add-to-cart'])) {
    ...
    try {
        // Получаем информацию о товаре с калориями из st_wc_product_meta_lookup
        $stmt = $pdo->prepare(
        ...
        if ($product) {
            ...
            if (isset($_SESSION['cart'][$product_id])) {
                ...
                $_SESSION['cart'][$product_id] = [
                    ...
                    'calories' => (float)$product['calories'], // Добавляем калории
                    'sku' => $product['sku'],
                    'quantity' => 1
                ];
            }
...
function get_product_image($image_id, $pdo) {
    if (!$image_id) return '/img/placeholder-product.jpg';

    try {
        $stmt = $pdo->prepare("SELECT guid FROM st_posts WHERE ID = ?");
        $stmt->execute([$image_id]);
        $image = $stmt->fetch(PDO::FETCH_ASSOC);
        return $image ? $image['guid'] : '/img/placeholder-product.jpg';
    } catch (PDOException $e) {
                    ...
                    <div class="products-grid">
                        <?php if ($total_products > 0): ?>
                            <?php foreach ($products as $product): ?>
                                ...
                                $product_image = get_product_image($product['image_id'], $pdo);
                                ?>
