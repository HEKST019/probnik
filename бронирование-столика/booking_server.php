...
// Функция для расчета времени истечения брони
function calculate_expiry_time($date, $slot) {
    // Создаем timestamp для начала временного слота
    $slotTime = strtotime($date . ' ' . $slot . ':00 Europe/Moscow');

    // Бронь истекает через 15 минут после начала временного слота
    $expiresAt = $slotTime + 3600; // 15 минут
...
function book_table($data) {
    ...
    // Рассчитываем время истечения брони (15 минут с начала временного слота)
    $expires_at = calculate_expiry_time($today, $slot);
    ...
    $booking = array(
        ...
        'slot_time' => strtotime($today . ' ' . $slot . ':00 Europe/Moscow')
    );

    $bookings[$today][$slot][$table] = $booking;

    if (save_bookings($bookings)) {
        log_booking("Table booked: $table at $slot by $user_name, expires at " . date('Y-m-d H:i:s', $expires_at));
...
function cancel_booking($data) {
    ...
    $table = isset($data['table']) ? $data['table'] : '';
    $user_id = isset($data['user_id']) ? $data['user_id'] : '';
    $is_admin_cancel = isset($data['is_admin_cancel']) ? $data['is_admin_cancel'] : false;
    $target_user_id = isset($data['target_user_id']) ? $data['target_user_id'] : '';
    ...
    // Проверяем, принадлежит ли бронь пользователю ИЛИ это отмена администратором
    if ($booking['user_id'] !== $user_id && !$is_admin_cancel) {
        return array('success' => false, 'error' => 'Вы не можете отменить чужую бронь');
    }

    // Для админской отмены проверяем, что отменяем именно ту бронь, которую хотели
    if ($is_admin_cancel && !empty($target_user_id) && $booking['user_id'] !== $target_user_id) {
        return array('success' => false, 'error' => 'Ошибка отмены: бронь не соответствует целевой');
    }
    ...
    if (save_bookings($bookings)) {
        if ($is_admin_cancel) {
            log_booking("Booking canceled by admin: $table at $slot (was booked by " . $booking['user_name'] . ")");
        } else {
